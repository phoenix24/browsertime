  ...
  async.whilst(
      ...
      function(cb) {
        ...
        runBrowser(options, function(err) {
          if (err) {
            log.error('Error running browser: %s', util.inspect(err));
          } else {
            successes++;
          }
          cb(null);
    ...
    async.series([
          ...
          function(callback) {
            ...
            try {
              ...
              var closer = self.browsers.getBrowser(options.browser);

              webdriver.promise.controlFlow().on('uncaughtException', function(e) {
                if (closer) {
                  // stop the service when getting uncaught
                  closer.stopService();
                }
                callback(e);
              });

              driver = self.browsers.getBrowser(options.browser).getDriver(options);
              ...
              pageLoadTimeout
                ...
                async.parallel(callbacks,
                    function(err, results) {
                      ...
                      driver.quit();
                      // hack for Chrome, stop the chromedriver
                      if (options.browser === 'chrome') {
                        self.browsers.getBrowser(options.browser).stopService();
                      }
                      callback();
                    });
              },
...
Browsertime.prototype._setupDefaults = function(options) {
  ...
  options.maxTries = options.maxTries || options.runs;
