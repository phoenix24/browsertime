...
Browsertime.prototype._killRunningChromeDriver = function(callback) {
  ...
  pgrep = exec('pgrep chromedriver',
    function(error) {
      if (error === null) {
        pkill = exec('pkill -9 chromedriver',
          function(e) {
            if (e) {
              self.log.info('Couldn\'t kill Chromedriver instance: ' + util.inspect(e));
            }
						else {
              self.log.info('Killed Chromedriver instance');
						}
            callback();
          });
        }
